...
class State:
    ...
    def generate_possible_moves(board, expansion_range):
        ...
        for r in range(game_settings.BOARD_ROW_COUNT):
            for c in range(game_settings.BOARD_COL_COUNT):
                if board[r][c] != game_settings.EMPTY:
                    neighbors = State.get_neighbors((r, c))
                    for neighbor in neighbors:
                        if State.is_valid_move(neighbor, board) and board[neighbor[0]][neighbor[1]] == game_settings.EMPTY:
                            possible_moves.add(neighbor)
    ...
    def combo_move(board, current_turn):
        ...
        for p_m_move in possible_moves:
            ...
            direction_pattern_tuples = State.get_direction_pattern_tuples(board, p_m_move, 4,
                                                                          current_turn)  # streak = 4 because we are checking length-5 patterns
            if (len(direction_pattern_tuples) > 0):
                for tuple in direction_pattern_tuples:
                    direction, pattern = tuple
        ...
        if len(matched_blocked_four_pattern_move_direction_list) >= 1:
            ...
            for p_m_move in matched_blocked_four_pattern_move_direction_list:
                ...
                direction_pattern_tuples = State.get_direction_pattern_tuples(board, blocked_four_move,
                    ...
                    for tuple in direction_pattern_tuples:
                        ...
                        if (pattern[move_pos_in_pattern] == current_turn):  # center pattern must be the current move
                            ...
                            check_left_pattern = pattern[1:5].count(current_turn) >= 3 and pattern[0:5].count(
                                game_settings.get_opponent(current_turn)) == 0
